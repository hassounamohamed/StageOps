...
$markRead = $conn->prepare('UPDATE notifications SET is_read = 1 WHERE user_id = ?');
if ($markRead) {
...
$notifStmt = $conn->prepare('SELECT message, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC');
$notif = null;
if ($notifStmt) {
...
render_layout_start('Admin Notifications', 'notifications');
?>
